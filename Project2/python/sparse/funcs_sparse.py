...
def r(coords,data,eps=10e-6):
    ...
    return np.array(coords2),np.array(data2)
...
def create_e_abij(occ, vir, e):
    ...
    return sparse.COO.from_numpy(e_abij)
...
def transform_one_body_elements_sparse(h, c):
    #_h = np.einsum("ip, jq, ij -> pq", c.conj(), c, h)
    _h = sparse.tensordot(c, h, axes=[(0),(1)])
    _h = sparse.tensordot(c.conj(),_h, axes=[(0),(1)])
    return _h
...
def MP2_sparse(int_2b, e_abij, occ, vir):
    ...
    #remove_small_elements(int_2b.coords,int_2b.data)
    #print ("MP2_sparse",t.density)
...
def CCD_solver_sparse(int_2b,e,occ,vir,t,E_ref,e_abij,opt=False):
    ...
    while (dE > eps) and ( counter < max_it ):
        ...
        E_old = CCD_energy
        counter += 1
        #t_.coords, t_.data = remove_small_elements(t_.coords,t_.data)
