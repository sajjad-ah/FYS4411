...
class HartreeFock:
    ...
    def scf(self, max_iters=100, tolerance=1e-9):
        ...
        print (u.data)
        print (u.coords)
        np.save("u_data",u.data)
        np.save("u_coords",u.coords)
        np.save("h_matrix",h)
        np.save("occ",occ)
        np.save("e",eps)
        return self.C
...
#system setup
num_part = 6
tdho = TwoDimensionalHarmonicOscillator(num_part,30,5,1,omega=1.0) #(num_of_particles,num_of_basis_functions,harmonic_oscillstor_size,grid_points)
tdho.setup_system()
...
np.save("C_data",C.data)
np.save("C_coords",C.coords)
np.save("primitive_occ",num_part)
np.save("num_basis",num_basis)
